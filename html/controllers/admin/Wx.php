 ...
 * @license     http://www.HXCMS.com/license
...
////////////////////////////////////USER////////////////////////////////////
    ...
    public function replyUsers($data1,$users,$all = false)
    {
        ...
        if ($post['type']) {
            ...
            $cdata['orther'] = dr_string2array($cdata['orther']);
    ...
    public function syncUsers()
    {
        ...
        if (!$ok) {
            $this->adminMsg("正在从微信服务端同步导入关注用户...", dr_url('admin/wx/syncUsers', array('ok' => 1)), 2, 2,1);
        }
        $key = $this->_get_access_token();
        $data = json_decode(@dr_catcher_data('https://api.weixin.qq.com/cgi-bin/user/get?access_token='.$key), true);
        if (!$data) {
        ...
        foreach ($data['data']['openid'] as $id) {
            $result = json_decode(@dr_catcher_data('https://api.weixin.qq.com/cgi-bin/user/info?access_token='.$key.'&openid='.$id), true);
